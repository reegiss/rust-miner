...
extern "C" __global__ void qhash_mine(
    ...
    for (int l = 0; l < N_LAYERS; l++) {
        ...
        // CNOT entanglement - optimized with proper temporary storage
        #pragma unroll
        for (int i = 0; i < N_QUBITS - 1; i++) {
            float coupling = 0.1f;
            float old_i = expectations[i];
            float old_i_plus_1 = expectations[i + 1];
            expectations[i] = old_i * (1.0f - coupling) + old_i_plus_1 * coupling;
            expectations[i + 1] = old_i_plus_1 * (1.0f - coupling) + old_i * coupling;
        }
    }
    ...
    // Convert hash to little-endian for comparison (Bitcoin/arith_uint256 semantics)
    uint8_t hash_le[32];
    #pragma unroll
    for (int i = 0; i < 32; i++) {
    ...
    // Compare from END backwards (index 31→0) for little-endian semantics
    // This matches Bitcoin's CompareTo: for (int i = WIDTH-1; i >= 0; i--)
